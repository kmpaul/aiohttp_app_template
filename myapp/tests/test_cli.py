...
def test_cli_defaults():
    ctx = cli.make_context(cli.name, args=[], auto_envvar_prefix='MYAPP')
...
def test_cli_host():
    ...
    ctx = cli.make_context(cli.name, args=['--host', params['host']], auto_envvar_prefix='MYAPP')
...
def test_cli_port():
    ...
    ctx = cli.make_context(
        cli.name, args=['--port', str(params['port'])], auto_envvar_prefix='MYAPP'
    )
    assert ctx.params == params
...
def test_cli_logging():
    ...
    ctx = cli.make_context(
        cli.name, args=['--logging', str(params['logging'])], auto_envvar_prefix='MYAPP'
    )
    assert ctx.params == params
...
def test_cli_mongouri():
    ...
    ctx = cli.make_context(
        cli.name, args=['--mongouri', params['mongouri']], auto_envvar_prefix='MYAPP'
    )
    assert ctx.params == params
...
def test_cli_mongodb():
    ...
    params['mongodb'] = 'test'
    ctx = cli.make_context(
        cli.name, args=['--mongodb', params['mongodb']], auto_envvar_prefix='MYAPP'
    )
    assert ctx.params == params
...
def test_cli_configfile(cli_configfile):
    ...
    params['port'] = 9999
    ctx = cli.make_context(cli.name, args=['--config', cli_configfile], auto_envvar_prefix='MYAPP')
    assert ctx.params == params
